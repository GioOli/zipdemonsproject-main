...
export default function AvariasGrid(props) {
  ...
  async function disponibilizarRows(items) {
    //get server response
    return await axios
      .put(
        "/itemFix",
      ...
      .catch(function (error) {
        ...
        console.log(error.response); //Backend
        // Bad credentials error
        return 404;
      });
  }

  function handleDisponibilizarAtivos() {
    // Active actives selected
  ...
  const popupMsg =
    "Tem a certeza que pretende colocar os ativos selecionados disponíveis de volta no sistema?";
        ...
        <div className={classes.grid}>
          ...
          <div>
            <PopUp
              ...
              actions={true}
              handleSend={handleDisponibilizarAtivos}
            />
          </div>
        </div>
      </div>
